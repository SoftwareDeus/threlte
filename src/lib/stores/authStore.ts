		...
		signIn: async (email: string, password: string): Promise<AuthResult> => {
			...
			if (!result.success) {
				update((state) => ({
					...state,
					loading: false,
					error: typeof result.error === 'string' ? result.error : 'Failed to sign in'
				}));
				return {
					success: false,
					error: typeof result.error === 'string' ? result.error : 'Failed to sign in'
		...
		signOut: async (): Promise<AuthResult> => {
			...
			if (!result.success) {
				update((state) => ({
					...state,
					loading: false,
					error: typeof result.error === 'string' ? result.error : 'Failed to sign out'
				}));
				return {
					success: false,
					error: typeof result.error === 'string' ? result.error : 'Failed to sign out'
		...
		deleteAccount: async (): Promise<AuthResult> => {
			...
			if (!result.success) {
				update((state) => ({
					...state,
					loading: false,
					error: typeof result.error === 'string' ? result.error : 'Failed to delete account'
				}));
				return {
					success: false,
					error: typeof result.error === 'string' ? result.error : 'Failed to delete account'
