...
export async function makeMove(lobbyId: string, playerName: string, move: any): Promise<GameState> {
    const response = await fetch(`/api/game/${lobbyId}`, {
        method: 'POST',
        headers: {
...
export async function updateTime(lobbyId: string, playerName: string, color: ChessColor): Promise<GameState> {
